...
- 入力モデル（ADR 0007）: Vim式のInsert/Normal/Selectモードは持たない（`Mode` enumは無い）。ただし入力状態は「直通」と「グローバルレイヤー（Ctrl+gで入る持続的なグローバル状態＝実質的なモード。`global_layer: bool`）」の2つを持つ。**「モードレス」「グローバルモードは無い」とは表現しないこと**（グローバルレイヤーは実態としてモード）。コピーモード・filerの複数選択は各ペイン内部の状態であり、グローバルな状態を追加しないこと
- 直通状態でアプリがキーを奪うのは禁止（トグルキーとLua `direct`登録分のみ例外）。グローバル操作を増やすときはグローバルレイヤー側に足すこと
