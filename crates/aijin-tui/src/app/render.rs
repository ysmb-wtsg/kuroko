...
use aijin_agent::{AgentPane, AgentStatus};
use aijin_core::layout::SplitDirection;
use aijin_core::theme;
use aijin_core::{FilePromptKind, Mode};
...
impl App {
    ...
    fn render_separators(
        ...
        for (direction, rect) in separators {
            let adjacent = focused_rect
                ...
                .unwrap_or(false);
            let color = if adjacent { t.border_focus } else { t.border_subtle };
            let style = Style::default().fg(color);
            let lines: Vec<Line> = match direction {
                SplitDirection::Vertical => {
                    (0..rect.height).map(|_| Line::from("│")).collect()
                }
    ...
    fn render_tab_bar(
        ...
        for (i, tab_id) in tabs.iter().enumerate() {
            ...
            let current_name = names.get(i)
                .and_then(|n| n.clone())
                .unwrap_or_else(|| {
                    self.panes.get(tab_id)
                        .map(|p| p.title().to_string())
                        .unwrap_or_else(|| "?".to_string())
                });

            // エージェントペインのステータスドット
            let status_dot = self.panes.get(tab_id)
            ...
            if is_renaming {
                ...
                push(&mut spans, &mut width_so_far, Span::styled(
                    format!(" {}:", i + 1),
                    Style::default().fg(t.text_primary).bg(bg).add_modifier(Modifier::BOLD),
                ));
                if input.is_empty() {
                    // カーソルはプレースホルダーの先頭に置く
                    ...
                    cursor_x = Some(area.x + width_so_far as u16);
                    push(&mut spans, &mut width_so_far, Span::styled(" ".to_string(), Style::default().bg(bg)));
                }
            } else {
                ...
                push(&mut spans, &mut width_so_far, Span::styled(format!(" {}:", i + 1), style));
                if let Some((dot, color)) = status_dot {
                    push(&mut spans, &mut width_so_far, Span::styled(dot.to_string(), style.fg(color)));
                }
                push(&mut spans, &mut width_so_far, Span::styled(format!("{} ", current_name), style));
            }
            if i + 1 < tabs.len() {
                push(&mut spans, &mut width_so_far, Span::styled("│".to_string(), Style::default().fg(t.border_subtle)));
            }
        }
    ...
    fn render_status_bar(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        let mode_style = Style::default().fg(t.text_on_accent).bg(mode_bg).add_modifier(Modifier::BOLD);
    ...
    fn render_file_preview(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        // 表示範囲の行を描画
        for (i, highlighted_line) in preview.lines.iter()
            .skip(scroll)
            ...
            .enumerate()
        {
            ...
            let line = if preview.is_text {
                ...
                let spans: Vec<Span> = highlighted_line.spans.iter()
    ...
    fn render_file_prompt(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        let accent = if is_delete { t.accent_warning } else { t.accent_primary };
    ...
    fn render_command_palette(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        // 入力行（実カーソルを入力末尾に表示）
        let prompt_span = Span::styled(": ", Style::default().fg(t.accent_primary).add_modifier(Modifier::BOLD));
        ...
        // 候補リスト
        for (i, &cmd_idx) in palette.filtered.iter()
            ...
            .enumerate()
        {
            ...
            let (name_style, desc_style) = if is_selected {
                (
                    Style::default().fg(t.text_primary).bg(t.surface_active).add_modifier(Modifier::BOLD),
...
mod render_tests {
    ...
    fn vertical_separator_adjacent_to_left_pane() {
        ...
        assert!(is_separator_adjacent(SplitDirection::Vertical, sep, focused));
    }
    ...
    fn horizontal_separator_adjacent_to_top_pane() {
        ...
        assert!(is_separator_adjacent(SplitDirection::Horizontal, sep, focused));
    }
