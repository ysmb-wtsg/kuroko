...
use aijin_core::{AijinError, PaneId};
...
impl PtyHandle {
    ...
    ///
    /// @param data - 書き込むバイト列
    pub fn write(&mut self, data: &[u8]) -> Result<(), AijinError> {
    ...
    pub fn spawn_with_command(
        ...
        let pair = pty_system
            ...
            .map_err(|e| AijinError::Pty(e.to_string()))?;
    ...
    /// @param cols - 新しい列数
    /// @param rows - 新しい行数
    pub fn resize(&self, cols: u16, rows: u16) -> Result<(), AijinError> {
        self.master
            .resize(PtySize {
                rows,
                cols,
