...
use aijin_core::{Action, Direction, FilePromptKind, LayoutNode, Mode, Pane, PaneId, SideContent};
use aijin_core::layout::SplitDirection;
use aijin_agent::{AgentPane, BuiltinProvider};
use aijin_filetree::FileTreePane;
use aijin_lua::{LuaRuntime, SharedKeymapRegistry};
use aijin_terminal::TerminalPane;
use aijin_terminal::pty_handle::PtyMessage;
...
impl App {
    /// 新しいAppインスタンスを生成する。
    /// Lua設定を先に読み込み、`aijin.opt.main_pane` に応じたメインペインを配置する。
    pub fn new() -> Self {
    ...
    fn try_lua_keymap(&self, key: &KeyEvent) -> Option<Vec<Action>> {
        ...
        // コールバック実行（Luaコールバック内でaijin.pane.* 等が呼ばれ、
                ...
                if self.git_panel_id.is_none() {
                    if which::which(&self.git_tool).is_err() {
                        self.overlay.set_status_message_with_level(
                            format!("{} not found. Install it or set aijin.opt.git_tool", self.git_tool),
                            MessageLevel::Warn,
                        );
                        return false;
                    }
...
/// XDG設定ディレクトリのパスを返す（~/.config/aijin/）
fn dirs_config_path() -> PathBuf {
    dirs_home().join(".config").join("aijin")
}
