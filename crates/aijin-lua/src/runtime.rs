//! Lua VMの初期化とaijin.* 名前空間の登録を行うモジュール。
...
use aijin_core::{Action, AijinError, Direction};
...
impl LuaRuntime {
    ...
    /// aijin.* 名前空間のAPI関数をLuaグローバルに登録する
    fn register_api(&self) -> mlua::Result<()> {
        let globals = self.lua.globals();

        let aijin = self.lua.create_table()?;

        // aijin.keymap
        let keymap = self.create_keymap_module()?;
        aijin.set("keymap", keymap)?;
    ...
    /// aijin.opt テーブルから文字列設定値を取得する。
    ///
    /// @param key - 取得するキー名
    /// @returns 値が存在すれば文字列、なければNone
    pub fn get_opt_string(&self, key: &str) -> Option<String> {
        let globals = self.lua.globals();
        let aijin: Table = globals.get("aijin").ok()?;
        let opt: Table = aijin.get("opt").ok()?;
    ...
    pub fn exec_file(&self, path: &std::path::Path) -> Result<(), AijinError> {
    ...
    /// コールバック内で `aijin.pane.*` 等のAPI呼び出しがあれば、
    /// action_tx経由でActionが送信される。
    ///
    /// @param registry_key - 実行するLuaコールバックのレジストリキー
    /// @returns 実行成功ならOk、エラーならErr
    pub fn exec_callback(&self, registry_key: &mlua::RegistryKey) -> Result<(), AijinError> {
...
mod tests {
    ...
    fn pane_toggle_terminal_sends_action() {
        ...
        runtime
            .lua
            .load(r#"aijin.pane.toggle("terminal")"#)
            .exec()
            .unwrap();
    ...
    fn pane_toggle_files_sends_action() {
        ...
        runtime
            .lua
            .load(r#"aijin.pane.toggle("files")"#)
            .exec()
            .unwrap();
        let action = rx.try_recv().unwrap();
    ...
    fn pane_toggle_unknown_sends_notify() {
        ...
        runtime
            .lua
            .load(r#"aijin.pane.toggle("unknown")"#)
            .exec()
            .unwrap();
        let action = rx.try_recv().unwrap();
    ...
    fn keymap_set_registers_binding() {
        ...
        let result = runtime
            .lua
            .load(r#"aijin.keymap.set("n", "q", function() end)"#)
            .exec();
        assert!(result.is_ok());
        // レジストリにNormalモードの "q" が登録されていることを確認
        let reg = runtime.keymap_registry.lock().unwrap();
        assert!(reg.get(aijin_core::Mode::Normal, "q").is_some());
    }
    ...
    fn keymap_set_expands_leader() {
        let (runtime, _rx) = setup();
        // リーダーキーをカンマに変更
        runtime.lua.load(r#"aijin.opt.leader = ",""#).exec().unwrap();
        runtime
            .lua
            .load(r#"aijin.keymap.set("n", "<leader>f", function() end)"#)
            .exec()
            .unwrap();
        let reg = runtime.keymap_registry.lock().unwrap();
        // "<leader>f" が ",f" に展開されていることを確認
        assert!(reg.get(aijin_core::Mode::Normal, ",f").is_some());
        assert!(reg.get(aijin_core::Mode::Normal, "<leader>f").is_none());
    }
    ...
    fn keymap_callback_sends_action() {
        ...
        runtime
            .lua
            .load(r#"aijin.keymap.set("n", "t", function() aijin.pane.toggle("terminal") end)"#)
            .exec()
            .unwrap();
        // コールバックを実行する（レジストリのロックを解放してからexec_callbackを呼ぶ）
        {
            let reg = runtime.keymap_registry.lock().unwrap();
            let entry = reg.get(aijin_core::Mode::Normal, "t").unwrap();
    ...
    fn exec_file_sets_option() {
        ...
        let path = dir.join("aijin_test_exec.lua");
        std::fs::write(&path, r#"aijin.opt.main_pane = "terminal""#).unwrap();
