...
impl Screen {
    fn text(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // Guard: after a resize the grid may be smaller than the
            // cursor position tracked by the parser.  Return early
            // instead of panicking on a missing cell.
            let Some(cur_cell) = self.grid().drawing_cell(pos) else {
                return;
            };

            if cur_cell.is_wide_continuation() {
            ...
            if let Some(cur_cell) = self.grid().drawing_cell(pos) {
                if cur_cell.is_wide() {
